...
// Anonymous = 60/hr; authenticated >= 5000/hr. Below this floor the token is
// degraded (revoked, scope-stripped, or unauthenticated).
...
// ghTokenCheckStep verifies GH_TOKEN before the phase runs:
//   - empty token   → needs_input — no credential configured
//   - 401           → needs_input — token bad
//   - <1000/hr      → needs_input — token degraded to anonymous
...
// token from startup auth (passed by main → factory). Empty means the step
// fails fast on the first run.
...
// + message only — the CONTROLLER owns the envelope (clears assignee, sets
// previous_assignee, appends ## Failure). Steps never write those.
...
// failed builds the retryable-failure Result shape (message only; controller
// owns retry policy and the failure envelope).
...
// error messages — limits both exposure and log noise from a large or
// hostile body.
