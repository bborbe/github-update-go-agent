...
- fix: the bulk-update loop checks `ctx.Done()` between commands, and each subprocess is logged on start as well as on completion (with duration). The start line is the point: this step exists because a long `go get` once hung invisibly, and a run that never returns must be distinguishable from one that was never attempted.
