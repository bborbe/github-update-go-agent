...
- fix: `runInspection` now classifies Go test-timeout hangs in the empty-on-error gate escalation — a `test timed out` signature in the captured output is named in the `NeedsInput` headline instead of the generic "no parseable findings" form (a hung gate and a fast lint error previously produced byte-identical escalations); non-timeout failures keep the generic message unchanged
...
- chore: update github.com/bborbe/vault-cli to v0.121.0
...
- change: remove the tag-triggered build workflow entirely — no CI build on tag push at all. Building + uploading the agent image is a purely local, deliberate action via `make buca` (VERSION=<tag>), tracked by the "Build and publish github-update-go-agent image on release" task (assignee bborbe), until a docker-build-agent takes over (decision 2026-08-27; supersedes v0.17.1's build-only validation)
