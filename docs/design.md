...
| Side effects | worktree @ current default-branch HEAD (resolved at run start); `git switch -c fix/update-go-<ref:7>` (ref = pinned filing SHA — deterministic replay guard); **update sequence per D2** (go-directive bump targeting image toolchain (D5) → excludes/replaces → `go get -u ./...` + `go mod tidy` → targeted `go get <pkg>@<fixed>` per plan → OSV/indirect cleanup) — **except on an advisory-driven task** (frontmatter carries an `advisory` block), where the `go get -u ./...` sweep is deliberately skipped: it takes every module to @latest, a superset of the advisory's `fixed_version` whenever a newer release exists, which makes the targeted pin that follows a no-op or a downgrade and destroys the "this advisory is fixed" reading of the diff. On that path the targeted `go get <pkg>@<fixed_version>` is the whole bump → **repair to green (bounded)**: Claude may edit code to fix compile/test breakage *caused by the bumps* (never unrelated refactors) → CHANGELOG bullet under `## Unreleased` → run ALL detected gate targets to exit 0 → commit (no attribution) → `git push --no-follow-tags -u origin <branch>` → `gh pr create` (`--draft` only when the target is draft) |
...
- **Auto-merge-on-policy is distinct from agent-initiated merge (decision 2026-08-18, see task [[Add Per-PR autoMergeOnGreen for End-to-End Agent PR Flow]])**: GitHub-native auto-merge — a ruleset-protected branch with required status checks + approvals where GitHub's policy engine executes the merge — does not violate the never-merge boundary above. The agent's capability set is unchanged: it still has no code path invoking `gh pr merge` (with or without `--auto`), `gh pr ready`, or `Tag`. The merge is performed by GitHub policy, not by the agent. Corollary constraint: the per-PR auto-merge opt-in (label / PR-body marker / config flag) must be detected and armed **outside** the agent's tool scope — the agent must never call `gh pr merge --auto` on its own PR, or the "no code path invokes `gh pr merge`" invariant breaks. Opt-in detection belongs to the controller/watcher side.
...
- **App auth (7.2a/7.2b):** App pair per stage — Contents R/W, Pull requests R/W, Metadata R, no Workflows, webhook off; dev installed on canary repo only, prod on `bborbe/*`.
  ...
  | prod | `Ben's Go Updater` (created 2026-07-21; installed on **All repositories**) | `4355244` | `148021284` | [TeamVault 9L6eKO](https://teamvault.benjamin-borbe.de/secrets/9L6eKO/) | PEM in Teamvault; App ID / Installation ID in CRD env. Ephemeral Job mints IAT once at startup: `resolveAuth` (lift from `github-dark-factory-agent` `main.go:264` — `githubapp.MintIAT` on `context.WithTimeout(WithoutCancel(ctx), 30s)`), then `os.Setenv("GH_TOKEN", …)` + `githubauth.NewGhAuthSetupGit(...)` with allowlisted subprocess env (`GH_TOKEN`,`HOME`,`PATH`). Raw `GH_TOKEN` accepted only for local `cmd/run-task`.
